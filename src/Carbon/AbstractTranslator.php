...
use Carbon\Message_Formatter\Message_Formatter_Mapper;
use Closure;
use Reflection_Exception;
use ReflectionFunction;
use ReflectionProperty;
use Symfony\Component\Translation\Formatter\Message_Formatter_Interface;
use Symfony\Component\Translation\Loader\Array_Loader;
use Symfony\Component\Translation\Translator as SymfonyTranslator;
use Throwable;
abstract class Abstract_Translator extends Symfony_Translator
{
    ...
    public function __construct($locale, ?Message_Formatter_Interface $formatter = null, $cache_dir = null, $debug = false)
    {
        $this->initialize($locale, $formatter, $cache_dir, $debug);
    }
    /**
     ...
     * @return $this
     */
    public function remove_directory(string $directory): static
    {
        $search = rtrim(strtr($directory, '\\', '/'), '/');
        return $this->set_directories(array_filter($this->get_directories(), static fn($item): bool => rtrim(strtr($item, '\\', '/'), '/') !== $search));
    }
    /**
    ...
    public function reset_messages(?string $locale = null): bool
    {
        if ($locale === null) {
            $this->messages = [];
            $this->catalogues = [];
            $this->modify_resources(static function (array $resources): array {
        ...
        $this->assert_valid_locale($locale);
        foreach ($this->get_directories() as $directory) {
            $file = \sprintf('%s/%s.php', rtrim((string) $directory, '\/'), $locale);
            $data = @include $file;
            if ($data !== false) {
                $this->messages[$locale] = $data;
                unset($this->catalogues[$locale]);
                $this->modify_resources(static function (array $resources) use ($locale): array {
                    unset($resources[$locale]);
                    return $resources;
                });
                $this->add_resource('array', $this->messages[$locale], $locale);
                return true;
            }
        }
        return false;
     ...
     */
    public function get_locales_files(string $prefix = ''): array
    {
        $files = [];
        foreach ($this->get_directories() as $directory) {
            foreach (self::get_php_files_in_directory(rtrim((string) $directory, '\/'), $prefix) as $file) {
                $files[] = $file;
            }
        }
     ...
     * @param string $prefix prefix required to filter result
     */
    public function get_available_locales(string $prefix = ''): array
    {
        return array_unique(array_merge(array_map(static fn(string $file): string => substr($file, strrpos($file, '/') + 1, -4), $this->get_locales_files($prefix)), array_keys($this->messages)));
    }
    protected function translate(?string $id, array $parameters = [], ?string $domain = null, ?string $locale = null): string
    {
        $domain ??= 'messages';
        $catalogue = $this->get_catalogue($locale);
        $format = $this instanceof Translator_Strong_Type_Interface ? $this->get_from_catalogue($catalogue, (string) $id, $domain) : $this->get_catalogue($locale)->get((string) $id, $domain);
        // @codeCoverageIgnore
        if ($format instanceof Closure) {
            // @codeCoverageIgnoreStart
            try {
                $count = (new ReflectionFunction($format))->get_number_of_required_parameters();
     ...
     *
     */
    protected function load_messages_from_file(string $locale): bool
    {
        return isset($this->messages[$locale]) || $this->reset_messages($locale);
    }
    /**
    ...
    {
        $this->load_messages_from_file($locale);
        $this->add_resource('array', $messages, $locale);
    ...
    {
        return $this->set_messages($this->get_locale(), $messages);
    }
    /**
     * Get messages of a locale, if none given, return all the
     * languages.
     */
    public function get_messages(?string $locale = null): array
    {
    ...
    {
        $locale = preg_replace_callback('/[-_]([a-z]{2,}|\d{2,})/', function ($matches): string {
            // _2-letters or YUE is a region, _3+-letters is a variant
            $upper = strtoupper((string) $matches[1]);
            if ($upper === 'YUE' || $upper === 'ISO' || \strlen($upper) <= static::REGION_CODE_LENGTH) {
                return "_{$upper}";
            }
            return '_' . ucfirst((string) $matches[1]);
        }, strtolower($locale));
        $previous_locale = $this->get_locale();
        if ($previous_locale === $locale && isset($this->messages[$locale])) {
            return;
        }
        unset(static::$singletons[$previous_locale]);
        if ($locale === 'auto') {
            $complete_locale = setlocale(LC_TIME, '0');
            $locale = preg_replace('/^([^_.-]+).*$/', '$1', $complete_locale);
            $locales = $this->get_available_locales($locale);
            $complete_locale_chunks = preg_split('/[_.-]+/', $complete_locale);
            $get_score = static fn($language) => self::compare_chunk_lists($complete_locale_chunks, preg_split('/[_.-]+/', (string) $language));
            usort($locales, static fn($first, $second): int => $get_score($second) <=> $get_score($first));
            $locale = $locales[0] ?? 'en';
        }
        ...
        if (str_contains((string) $locale, '_') && !\in_array($locale, self::get_internally_supported_locales(), true) && $this->load_messages_from_file($macro_locale = preg_replace('/^([^_]+).*$/', '$1', (string) $locale))) {
            parent::set_locale($macro_locale);
        }
        if (!$this->load_messages_from_file($locale) && !$this->initializing) {
            return;
        }
    ...
    private function initialize($locale, ?Message_Formatter_Interface $formatter = null, $cache_dir = null, $debug = false): void
    {
        parent::set_locale($locale);
        $this->initializing = true;
        $this->directories = [self::get_default_lang_directory()];
        $this->add_loader('array', new Array_Loader());
        parent::__construct($locale, new Message_Formatter_Mapper($formatter), $cache_dir, $debug);
        $this->initializing = false;
    }
    private static function compare_chunk_lists($reference_chunks, array $chunks): int
    {
        $score = 0;
        foreach ($reference_chunks as $index => $chunk) {
            if (!isset($chunks[$index])) {
                $score++;
                continue;
            }
            if (strtolower($chunks[$index]) === strtolower((string) $chunk)) {
                $score += 10;
            }
        }
        return $score;
    ...
    /** @codeCoverageIgnore */
    private function modify_resources(callable $callback): void
    {
        try {
            $resources_property = new ReflectionProperty(Symfony_Translator::class, 'resources');
            $resources = $resources_property->get_value($this);
            $resources_property->set_value($this, $callback($resources));
        } catch (Throwable) {
            // Clear resources if available, if not, then nothing to clean
        }
    }
    private static function get_php_files_in_directory(string $directory, string $prefix): array
    {
        if ($directory !== self::get_default_lang_directory()) {
        ...
        $locales = self::get_internally_supported_locales();
        if ($prefix !== '') {
            $locales = array_values(array_filter(self::get_internally_supported_locales(), static fn(string $locale): bool => str_starts_with($locale, $prefix)));
        }
        return array_map(static fn(string $locale): string => "{$directory}/{$locale}.php", $locales);
    }
    private static function get_default_lang_directory(): string
    {
        return __DIR__ . '/Lang';
    }
    /** @return list<string> */
    private static function get_internally_supported_locales(): array
    {
